...
public class TreeTraversals {
    ...
    static void preorderPrint(Tree node) {
        ...
        final Iterator iter = node.children.iterator();
        while (iter.hasNext()) {
    ...
    static boolean dfs(Tree root) {
        ...
        stack.push(root);
        while (!stack.isEmpty()) {
            final Tree node = (Tree) stack.pop();
    ...
    /**
     * Performs a depth-first search, starting from the given node, for any node
     * that satisfies the condition isGoal(node), and prints the path from the
     * goal back to the root.
     *
     * @param root
     *            The root of the tree or subtree to be searched.
     * @return True if a goal node could be found.
    ...
    static boolean recursiveDfs(Tree node) {
        ...
        final Iterator children = node.children.iterator();
        while (children.hasNext()) {
            final Tree child = (Tree) children.next();
    ...
    static boolean bfs(Tree root) {
        ...
        queue.add(root);
        while (!queue.isEmpty()) {
            final Tree node = (Tree) queue.get();
            System.out.print("Checking node " + node.value);
            if (isGoal(node)) {
                System.out.println("\nFound goal node " + node.value);
                return true;
    ...
    private static class Queue {
        ...
        /** Enqueues a whole collection of objects. */
